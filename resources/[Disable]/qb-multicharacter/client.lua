...
-- custom_auth / custom_charcreator replace the classic character flow on this server.
-- When they are running, qb-multicharacter must not grab NUI focus on boot.
local customFlow = GetResourceState('custom_auth') == 'started' or GetResourceState('custom_auth') == 'starting'

CreateThread(function()
    if customFlow then return end
    while true do
        Wait(0)
